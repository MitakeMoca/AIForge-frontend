<template>
  <div class="body">
    <el-scrollbar>
      <div class="project-item" @click="showConfig(item.projectId)" v-for="(item,index) in projectList" :key="index">
        ...
        <span class="status" :class="{waiting : item.status === 'wait', finished: item.status === 'finished',
         init: item.status === 'init', stopped: item.status === 'stopped', }">
        状态: {{item.status}}
      </span>
        <run-icon class="run-icon" @click.stop="runProject(item)"></run-icon>
        <big-delete-icon class="delete-icon" @click.stop="deleteProject = item;deleteDialog = true"></big-delete-icon>
