        ...
        <div class="file-list-content">
          <div class="main-files">
            <div  class="main-file-item">
              ...
              <span  class="file-size">({{ dataset.dataSize }}MB)</span>
          ...
          <div class="detailed-files">
            <table>
              ...
              <tbody>
                <tr v-for="file in limitedFiles" :key="file.name">
                  <td>{{ file.name }}</td>
                  <td>{{ file.size }}</td>
...
import { findDatasetbyid } from '@/service/dataset';
...
const fetchUserName = async (userId) => {
  ...
  try {
    const response = await axios.post("/User/findbyuserid",{UserId:  userId});
    if (dataset.value) {
...
const download = async () => {
  try {
    console.log(idValue)
    const response = await axios.post("/Dataset/downloadFiles",
    {
      DatasetId: idValue
    },
    {
...
const limitedFiles = computed(() => { return files.value.slice(0, 6); });
