  ...
  <div class="dataset-container">
    ...
    <div class="nav-tabs">
      <div class="nav-tabs-inner">
        <div class="nav-links">
          <a href="#" :class="{'nav-link': true, 'active': selectedTab === '公开数据集'}" @click="thePublic()">公开数据集</a>
          <a href="#" :class="{'nav-link': true, 'active': selectedTab === '我的数据集'}" @click="mine()">我的数据集</a>
        </div>
    ...
    <!-- Filters and Search -->
    <div class="filters">
      <select v-model="category" class="filter-select" @change="toggleTagsDropdown">
    ...
    <div class="category-tags">
      <button :class="{'category-tag':true,'active':category === '全部'}" @click="all()">全部</button>
      <button :class="{'category-tag':true,'active':category === '计算机视觉'}" @click="computerVision()">计算机视觉</button>
      <button :class="{'category-tag':true,'active':category === '自然语言处理'}" @click="naturalLanguageProcessing()">自然语言处理</button>
      <button :class="{'category-tag':true,'active':category === '推荐系统'}" @click="recommendationSystem()">推荐系统</button>
      <button :class="{'category-tag':true,'active':category === '机器学习'}" @click="machineLearning()">机器学习</button>
    </div>

    <!-- Dataset Listings -->
    
    <div class="dataset-list" v-if="allDataSetIsV">
      <div v-for="dataset in datasetByCategory" :key="dataset.dataSetId" class="dataset-card">
        <div class="dataset-header">
          <div class="dataset-title-container">
            <span class="dataset-tag">精</span>
            <h3 class="dataset-title" @click="goToDataPage(dataset.dataSetId)">{{ dataset.dataSetName}}</h3>
          </div>
          <p class="dataset-description">介绍：{{ dataset.truncDes }}</p>
          <div class="dataset-meta">
            <div class="dataset-author">
              <UserIcon class="meta-icon" />
              <span>作者：{{ dataset.userName }}</span>
            </div>
    ...
    <div class="mydataset-list" v-if="myDataIsVisible">
      <div v-for="dataset in myDatasetsByCategory" :key="dataset.dataSetId" class="dataset-card">
        <div class="dataset-header">
          <div class="dataset-title-container">
            <span class="dataset-tag">精</span>
            <h3 class="dataset-title" @click="goToDataPage(dataset.dataSetId)">{{ dataset.dataSetName}}</h3>
          </div>
          <p class="dataset-description">介绍：{{ dataset.truncDes}} (详情请点击)</p>
          <div class="dataset-meta">
            <div class="dataset-author">
              <UserIcon class="meta-icon" />
              <span>作者：{{ dataset.userName }}</span>
...
import {findAllDataset,getAllPublicDataset} from '@/service/dataset';
import { getLocal } from "@/utils/local";
const router = useRouter();
  
//全部的标签
const tags = [
  '计算机视觉', '目标检测', '语义分割', '实例分割', '图像分类',
  '关键点检测', 'OCR识别', '人体姿态估计', '图像超分辨率', '风格迁移',
  '图像翻译', '动作迁移', '生成', '人脸识别', '自然语言处理',
  '舆情分析', '智能问答', '对话系统', '观点抽取', '文本审核',
  '文言语音', '文言标签', '文本纠错', '知识图谱','推荐系统','机器学习'
]
...
const fetchAllDataset = async () => {
  try {
    ...
    datasets.value = datasets.value.map(dataset => ({
      ...dataset,
      truncDes: dataset.introduction.length > 60
    ...
    for (const data of datasets.value) {
      ...
      try {
        const userResponse = await axios.post("/User/findbyuserid",{UserId: data.userId}); // 根据 UserId 获取 UserName

        data.userName = userResponse.data.username; // 将 UserName 添加到数据集中
        
      } catch (userErr) {
        console.error(`获取 UserName 失败, UserId: ${data.userId}`, userErr);
      }
    }
  } catch (err) {
...
const myDatasets = computed(() => {
  return datasets.value.filter(item => item.userId === getLocal('userId'));
});
...
const showResults = () => {
  ...
  filteredDatasets.value = datasets.value.filter(dataset => {
    return dataset.dataSetName.includes(query.value) ||
