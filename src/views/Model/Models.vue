					...
					<div class="type_choose">
						<Space vertical>
							...
							<div>
								<el-checkbox-group
									v-model="value"
									@change="handleCheckedTagsChangeMyModels"
								>
					...
					<div class="scrollable-pipeline-list">
						<el-row gutter="20">
							<el-col
								class="model-col"
								:span="24"
								v-for="(model, index) in filteredPipelines"
								:key="index"
							>
								<el-card
									class="pipeline-card"
...
const confirmSetPublic = (id) => {
	ElMessageBox.confirm(
		'确定要将此模型设置为“公开”吗？设为“公开”后，模型将对其他用户可见',
		'设置确认',
		{
			confirmButtonText: '确定',
			cancelButtonText: '取消',
...
const confirmSetPrivate = (id) => {
	ElMessageBox.confirm(
		'确定要将此模型设置为“私有”吗？设为“私有”后，模型自其他用户不可见',
		'设置确认',
		{
			confirmButtonText: '确定',
			cancelButtonText: '取消',
...
async function fetchMyModels() {
	try {
		...
		myModels.value = modelsWithPendingTags.map((model) => {
			const tagsResult = tagsResults.find(
...
const filteredMyModels = computed(() => {
	...
	if (value.value.length > 0) {
		const selectedLabels = value.value.map((val) =>
			tagMap.value.get(Number(val)),
		);
...
watch(
	...
	(newValue) => {
		...
		indeterminate.value =
			newValue.length > 0 && newValue.length < options.value.length;
	},
	{ deep: true },
);
