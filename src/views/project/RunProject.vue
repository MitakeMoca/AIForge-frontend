<template>
  <el-container style="height: 100vh">
    ...
    <div class="action-buttons">
      ...
      <el-button type="info" @click="startDocker('test')"> 开始测试 </el-button>
      <el-button type="danger" @click="deriveModel()"> 导出模型 </el-button>
    </div>
    ...
    <el-aside width="30%">
      <div class="project-filter-container">
        ...
        <FolderTree :dataSource="fileData" @file-clicked="handleFileClicked" />
      </div>
      <div class="download">
          ...
          <div class="tab-content">
            ...
            <div v-if="fileContent">
              ...
              <div v-if="isImageFile" class="file-content">
                <img :src="fileContent" alt="image" class="file-image" />
              </div>
            </div>
          </div>
        </el-tab-pane>
            ...
            <el-main class="chat-main">
              ...
              >
                ...
                <img
                  ...
                  class="message-image"
                />
                <p v-if="msg.type === 'user'" class="message-user">
                  {{ msg.content }}
                </p>
              </div>
            </el-main>
...
const startDocker = async (command) => {
  ...
  const findPathByProjectIdResponse = await findPathByProjectId({
    ProjectId: Number(stata.project.projectId),
  });
  console.log(findPathByProjectIdResponse.data);
  console.log(findPathByProjectIdResponse);
  if (
    findPathByProjectIdResponse.data == null ||
    findPathByProjectIdResponse.data.length == 0
  ) {
  ...
  const findHyparaByPathResponse = await findHyparaByPath({
    StorePath: findPathByProjectIdResponse.data[0],
  });
  if (
  ...
  try {
    ...
    if (confirm) {
      ...
      if (project2modelResponse.resultCode === 200) {
        ...
        ElMessage.error(`导出模型失败: ${project2modelResponse.message}`);
      }
    }
  } catch (error) {
    // 用户点击 "取消" 或出现异常
